...
The Docker Compose file applies a `400m` memory and `0.2` CPU limit. The limits constrain the app container; image generation compute and temporary provider-side processing happen in Gemini and Replicate. The app defaults to 3MB per uploaded image and allows only one active generation at a time.
