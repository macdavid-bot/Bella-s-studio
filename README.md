...
The Docker Compose file applies a `400m` memory and `0.2` CPU limit. The limits constrain the app container; image generation compute and temporary provider-side processing happen in Gemini and Replicate. Uploads stream to VPS storage instead of being buffered entirely in RAM, and the app allows only one active generation at a time.

There is no app-level per-image size limit. The app accepts one target and up to two references, stores incoming files on the VPS, and applies the 10GB storage retention policy. A reverse proxy may still have its own request-size setting; if you use one, configure it for the image sizes you expect.